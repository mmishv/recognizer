...
class StructuralAnalysis:
    ...
    def __main_parsing(self, symbols):
        ...
        helpers.debug(
            "\n[parser.py] __main_parsing() | \
            STARTING symbol index: %d "
            % sstart
        )
        helpers.debug(
            "[parser.py] __main_parsing() | \
            STARTING symbol label: %s "
            % listin[sstart]["label"]
        )
    ...
    def __overlap(self, symbolIndex, top, bottom, listin):
        ...
        while listIndex < n and listin[listIndex]["xmin"] < listin[symbolIndex]["xmax"]:
            ...
            print(
                "[parser.py] __overlap() | ... \
                label: ",
                listin[listIndex]["label"],
            )
            print(
                "[parser.py] __overlap() | ... \
                centroid: ",
                listin[listIndex]["centroid"],
            )
            print(
                "[parser.py] __overlap() | ... \
                xmin: ",
    ...
    def __hor(self, listin, index):
        ...
        if label in range(10, 17):
            ...
            helpers.debug(
                "[parser.py] __hor() | top: %d, bottom: %d, \
                left: %d, right: %d"
            ...
            for s in range(0, len(listin)):
                ...
                if not checked:
                    ...
                    helpers.debug(
                        ...
                        ymin: %s ymax: %s"
                        % (symbol["xmin"], symbol["xmax"], symbol["ymin"], symbol["ymax"])
                    )

                    if (
                        symbol["centroid"][0] >= left
                        and symbol["centroid"][0] <= right
                        and symbol["centroid"][1] <= bottom
                        and symbol["centroid"][1] >= top
                    ):
                        helpers.debug(
                            "[parser.py] __hor() | \
                                ......... founded: %s"
